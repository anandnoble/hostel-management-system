...
create table public.rooms (
    ...
    room_type text not null, -- e.g., "AC Single", "Non-AC Double"
...
create table public.students (
    ...
    student_id_number text not null, -- Student Roll Number/Reg ID
    dob date,
    gender text,
    course text,
    department text,
    ...
    joining_date date,
    leaving_date date,
    unique(id) -- one student record per profile
);
...
create table public.room_allocations (
    ...
    status text not null default 'Active', -- 'Active', 'Transferred', 'Vacated'
    notes text,
...
-- Complaint Assignments (assigning staff to tasks)
...
-- Complaint Comments (Work notes / updates)
...
create table public.announcements (
    ...
    target_audience text not null default 'ALL', -- 'ALL', 'HOSTEL', 'BUILDING'
...
create index idx_hostels_org_id on public.hostels(organization_id);
...
create index idx_room_allocations_bed on public.room_allocations(bed_id);
create index idx_fee_invoices_student on public.fee_invoices(student_id);
create index idx_payments_invoice on public.payments(fee_invoice_id);
create index idx_complaints_student on public.complaints(student_id);
create index idx_complaint_assignments_staff on public.complaint_assignments(staff_id);
...
-- 5. Helper Function to Get Current User's Organization ID
...
-- Organization policies: Users can only read their own organization
...
using (
    ...
    and public.current_user_role() in ('SUPER_ADMIN', 'HOSTEL_ADMIN')
);
...
-- Hostel structures policies (Hostels, Buildings, Floors, Rooms, Beds)
-- All users in the organization can select structural data
create policy "Users can view hostels in their org"
on public.hostels for select
using (organization_id = public.current_user_org_id());

create policy "Admins can manage hostels in their org"
...
-- Floors policies (similarly filtered by building -> hostel -> org)
...
using (
    exists (
        ...
        where b.id = building_id and h.organization_id = public.current_user_org_id()
    )
);
...
using (
    exists (
        ...
        join public.hostels h on b.hostel_id = h.id
        where f.id = floor_id and h.organization_id = public.current_user_org_id()
    )
);
...
using (
    exists (
        ...
        join public.buildings b on f.building_id = b.id
        join public.hostels h on b.hostel_id = h.id
        where r.id = room_id and h.organization_id = public.current_user_org_id()
    )
);
...
-- Student policies
create policy "Users can view student records in their org"
on public.students for select
using (
    exists (
        select 1 from public.profiles p 
        where p.id = students.id and p.organization_id = public.current_user_org_id()
    )
);
...
create policy "Students can update their own student record details"
on public.students for update
using (id = auth.uid());

-- Room allocations policies
create policy "Users can view room allocations in their org"
on public.room_allocations for select
using (
    exists (
        select 1 from public.profiles p 
        where p.id = student_id and p.organization_id = public.current_user_org_id()
    )
);

create policy "Admins can manage room allocations"
...
using (
    exists (
        select 1 from public.profiles p 
        where p.id = student_id and p.organization_id = public.current_user_org_id()
    ) and public.current_user_role() in ('SUPER_ADMIN', 'HOSTEL_ADMIN', 'ACCOUNTANT')
);
...
-- Complaints policies
create policy "Students can view and manage their own complaints"
on public.complaints for all
using (
    student_id = auth.uid()
);

create policy "Admins can view and manage all complaints"
on public.complaints for all
using (
    exists (
        select 1 from public.profiles p 
...
create policy "Maintenance staff can view complaints assigned to them"
on public.complaints for select
using (
    public.current_user_role() = 'MAINTENANCE_STAFF'::public.user_role
    and exists (
        select 1 from public.complaint_assignments ca
        where ca.complaint_id = complaints.id and ca.staff_id = auth.uid()
    )
);

create policy "Maintenance staff can update status of complaints assigned to them"
...
using (
    exists (
        select 1 from public.profiles p 
        where p.id = staff_id and p.organization_id = public.current_user_org_id()
    )
);
...
create policy "View comments for visible complaints"
...
create policy "Post comments for visible complaints"
...
-- 7. Define DB Functions and Triggers for Actions

-- Profile auto-handling trigger on auth sign up
...
-- Trigger to log audits when critical tables change
...
-- Add audit logging triggers to critical tables
